...
for i = 1:NB_CAMERA
    for j = 1:NB_PERSON
        background_model{i,j} = zeros(Camera{i,rep(i)}.Height, Camera{i,rep(i)}.Width);
        if ~isempty(Camera{i,rep(i)})
            for k = 1:interest(i)
                frame = double(im2gray(readFrame(Camera{i,rep(i)})));
                background_model{i,j} = background_model{i,j} + frame;
            end
            background_model{i,j} = mat2gray(background_model{i,j} / interest(i));
            Camera{i,rep(i)}.CurrentTime = 0.0;
        end
    end
end
...
THRESH_RHO    = 0.98;      OMEGA = 3;
ALPHA         = 0.01 ; HAL_OMEGA = round(OMEGA / 2);
THRESH_MIN_PIXEL = [350, 400, 150, 300];
THRESH_MAX_PIXEL = [600, 600, 600, 600];
        ...
        while hasFrame(Camera{cam,pers})
            ...
            for h = 1:IMG_SIZE(1) - OMEGA + 1
                for w = 1:IMG_SIZE(2) - OMEGA + 1
                    ...
                    if rho < THRESH_RHO
                        mask(h+HAL_OMEGA, w+HAL_OMEGA) = 1;
                    end
                end
            end
...
for cam = 1:NB_CAMERA
    ...
    for i = 1:NB_PERSON
        ...
        subplot(3,3,i);
        plot(D{cam,i});
        hold on; plot(ones(1,length(D{cam,i})) * THRESH_MIN_PIXEL(cam), 'r--'); hold off;
        hold on; plot(ones(1,length(D{cam,i})) * THRESH_MAX_PIXEL(cam), 'r--'); hold off;
        xlim([1, length(D{cam,i})]);
